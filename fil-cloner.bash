...
for arg in "$@"; do \
  ...
  if /usr/bin/lsblk -n -o mountpoint "${drvMatches[0]}" | grep -v '^$' ; then
    echo "Drive with serial number '$arg' seems to be mounted as listed above"
    exit 1
  fi
  if /usr/bin/lsblk -n -o name,type "${drvMatches[0]}" | grep -vE '(disk|part)$' ; then
...
scratchfn="$( /usr/bin/mktemp -t tmp.XXXXXXXXXX )"
...
onEnd() {
  ...
  curl -s "https://fil-discover-drive-clone.s3-ap-east-1.amazonaws.com/$(date +'%s.%N')_${ex}_$( <<<"${srcDrive} ${dstDrives[@]}" /usr/bin/sed 's/ /_/g' | /usr/bin/sed 's/\/dev\/disk\/by-id\///g' | /usr/bin/sed 's/\//-/g' )" \
...
exec 3>&2
exec 2> >( /usr/bin/tee -a "$scratchfn" >&3)
exec 1> >( /usr/bin/tee -a "$scratchfn" >&3)

nl=$'\n'
IFS="$nl"
now=$( /usr/bin/date -u +'%H:%M:%S' )
...
IFS=" "
finCmd="/usr/bin/dd bs=32M status=progress if=$srcDrive"
for d in "${dstDrives[@]}"; do
  finCmd="$finCmd | /usr/bin/tee >( /usr/bin/dd bs=32M status=none of=$d)"
done
